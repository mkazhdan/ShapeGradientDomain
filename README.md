<CENTER><H2>Shape Gradient Domain (Version 4.5)</H2></CENTER>
<CENTER>
...
<A HREF="https://www.cs.jhu.edu/~misha/Code/ShapeGradientDomain/Version4.5/ShapeGradientDomain.x64.zip">Windows (x64) Executables</A><BR>
<A href="https://www.cs.jhu.edu/~misha/Code/ShapeGradientDomain/Version4.5/ShapeGradientDomain.zip">Source Code</A><br>
...
where Id. is the identity matrix, &Kappa;<sup>2</sup> is the diagonal matrix whose entries are the squares of the principal curature values, and &epsilon; is the curvature weight. (When &epsilon;=0, this reproduces the Riemannian metric given by the embedding.)<br>
...
<LI> For robustness, the implementation supports pre-smoothing the normals before estimating curvatures. This can either be done directly by smoothing the individual channels or by performing the  harmonic smoothing described in [<A href="https://www.cs.jhu.edu/~misha/MyPapers/SIG16.pdf">Prada et al. 2016</A>]. In either case, this amounts to minimizing an energy of the form:<br>
<CENTER>
...
<DT>[<b>--anisotropic</B>]
<DD> If this flag is enabled, the metric is scaled anisotropically along the two principal curvature directions -- &Kappa;=Diag(&kappa;<sub>1</sub>,&kappa;<sub>2</sub>). Otherwise, the metric is scaled by the same amount in both diretions -- &Kappa;<sup>2</sup> &larr; tr(&Kappa;<sup>2</sup>)&sdot;Id..


<DT>[<b>--hNormals</b>]
<DD> If this flag is enabled, the harmonic smoothing of [<A href="https://www.cs.jhu.edu/~misha/MyPapers/SIG16.pdf">Prada et al. 2016</A>] is used to pre-process the normals before estimating curvatures. (If this flag is enabled, the default value of <CODE>--nWeight</CODE> may need to be increased.)
...
If you are using Eigen and your implementation is backed by <A HREF="https://software.intel.com/en-us/intel-mkl/">Intel's Math Kernel Library</A> (see discussion <A HREF="https://eigen.tuxfamily.org/dox/TopicUsingIntelMKL.html">here</A>), enable the <CODE>EIGEN_USE_MKL_ALL</CODE> macro by defining it in the file <CODE>PreProcessor.h</CODE>. (The two versions of the <A HREF="https://www.cs.jhu.edu/~misha/Code/ShapeGradientDomain/Version4.5/ShapeGradientDomain.x64.zip">Windows executables</A> are compiled with and without MKL support.)
</UL>
